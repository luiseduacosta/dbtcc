...
if (!empty($registro_aluno1)) {
    /* Primeiro verifico se já está cadastrado */
    $sql_tcc = "select registro, num_monografia from tcc_alunos where registro=$registro_aluno1";
    $res_tcc = $db->Execute($sql_tcc);
    $registro_tcc = $res_tcc->fields['registro'];
    $id_monografia = $res_tcc->fields['num_monografia'];
    if ($registro_tcc) {
        echo "Vai para atualização <br>";
        header("Location:../visualizar/ver_monografia.php?codigo=$id_monografia");
        die("Estudante já cadastrado com TCC");
    } else {
...
if ($catalogo) {
    ...
    if ($resposta_catalogo === null)
        die("Não foi possível consultar a tabela tcc_alunos");
...
if (empty($id_areaProfessor)) {
    $id_areaProfessor = 0;
    $error[$numerros] = "É obrigatório inserir a área do professor selecionado";
    // $numerros++;
}
